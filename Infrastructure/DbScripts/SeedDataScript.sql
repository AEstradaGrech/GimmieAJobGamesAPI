...
VALUES
('GetStudioGamesByName', 'SELECT g.Title, g.Genre, g.PEGI, g.Price, p.PromoDesc AS Promotion FROM Games AS g INNER JOIN StudioGame AS relTb ON relTb.GameId = g.Id  INNER JOIN Studios AS s ON relTb.StudioId = s.Id LEFT OUTER JOIN GamePromotion AS promoJoin ON promoJoin.GameId = g.Id LEFT JOIN Promotions AS p ON promoJoin.PromotionId = p.Id WHERE s.StudioName LIKE \'{studioName}%\';'),
('GetGamesByPEGI', 'SELECT g.Title, g.Genre, g.PEGI, g.Price, p.PromoDesc AS Promotion FROM Games AS g LEFT OUTER JOIN GamePromotion AS promoJoin ON promoJoin.Id = g.Id LEFT JOIN Promotions AS p ON promoJoin.PromotionId = p.Id WHERE g.PEGI = \'{PEGI}\';'),
...
SELECT g.Title, g.Genre, g.PEGI, g.Price, p.PromoDesc 
FROM Games AS g 
INNER JOIN StudioGame AS relTb ON relTb.GameId = g.Id 
INNER JOIN Studios AS s ON relTb.StudioId = s.Id
LEFT OUTER JOIN GamePromotion AS promoJoin ON promoJoin.GameId = g.Id 
LEFT JOIN Promotions AS p ON promoJoin.PromotionId = p.Id 
